...
def create_decoder_block(in_channels, out_channels, kernel_size, wn, bn, layers, activation=nn.ReLU, final_layer=False):
    decoder = []
    for i in range(layers):
        _in = in_channels
        _out = in_channels
...
def create_encoder(in_channels, filters, kernel_size, wn, bn, layers, activation=nn.ReLU):
    encoder = []
...
def create_decoder(out_channels, filters, kernel_size, wn, bn, layers, activation=nn.ReLU):
    decoder = []
    for i in range(len(filters)):
        if i == 0:
...
class UNetEx(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, filters, layers,
                 weight_norm, batch_norm, activation=nn.ReLU, final_activation=None):
        super().__init__()
